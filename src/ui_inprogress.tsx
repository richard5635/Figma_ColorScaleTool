...
class App extends React.Component<{}, any> {
  ...
  onCheckColorUponEnter(e, id) {
    if (e.keyCode === 13) {
      // Need to trigger blur. How?
      e.target.blur();
      // Validation will happen in checkColor. Triggered by onBlur() in html tag
    }
  }

  checkColor(e, id) {
    let value = e.target.value;
    console.log("Color check: " + value);

    // Validation
    /*
    1. Check if it is hex if yes, do the conversion
    2. Check if it is math formula. If yes, calculate it.
    3. If fail, return to basic saved value.
     */
    if (id === "hex") {
      // Validation
      console.log(value);
      ...
      // Check if it is pure number or a math formula. Turn it into a number.
      
      let valid = true;
      // Calculate the value. If the number or the formula is not valid, valid = false.

      // Check if
      if (id.substr(0, 3) === "rgb") {
        value = parseInt(value);
        ...
        if(!valid){
          let tempRGB = this.state.rgb;
          tempRGB[index] = tempColor;
          this.setState({rgb: tempRGB});
          return;
        }
        ...
        <div className="content">
          ...
          <div className="line-container">
            ...
            <input
              ...
              onFocus={(e) => this.saveVariable(e)}
              onBlur={(e) => this.checkColor(e, "rgb-r")}
              onKeyUp={(e) => this.onCheckColorUponEnter(e, "rgb-r")}
            />
            <input
              className="text-input text-input--short"
              id="rgb-g"
          ...
          <div className="line-container">
            <button id="save-color" onClick={this.saveColor}>
              Save Color
            </button>
          </div>
          <div className="line-container">
          ...
          <div className="line-container">
            ...
            {colorsSaved.map((col, index) => {
              return (
                <ColorBox key={"colorbox-" + index} color={col}>
                  a
                </ColorBox>
              );
            })}
